    ...
    <ul class="nav navbar-nav flex-row">
      <li class="nav-item mr-auto">
        <a class="navbar-brand" href="/dashboard">
          ...
          <h2 class="brand-text mb-0"> Webix </h2>
        </a>
      </li>
   ...
   <ul class="navigation navigation-main" id="main-menu-navigation" data-menu="menu-navigation">
      ...
      <li class="nav-item {{ (request()->is($menu->slug)) ? 'active' : '' }} {{ $custom_classes }}">
        <a href="{{ $menu->url }}">
          ...
          @php
          $badgeClasses = "badge badge-pill badge-danger float-right";
          $deputy_badge = "";
          @endphp
          @if (isset($menu->badge))

          <span
            class="{{ isset($menu->badgeClass) ? $menu->badgeClass.' test' : $badgeClasses.' notTest' }} {{ ($deputy_badge != "") ? $deputy_badge : '' }} ">{{$menu->badge}}</span>
          @endif
          @if ($menu->name == "I'm a Deputy For")
          @php $deputy_badge = "pending_deputy" @endphp
          <span class=" {{ ($deputy_badge != "") ? $badgeClasses.' '.$deputy_badge : '' }} "></span>
          @endif
        </a>
        @if(isset($menu->submenu))
        @include('panels/submenu', ['menu' => $menu->submenu])
